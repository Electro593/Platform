...
#define FONT_FUNCS \
    EXPORT(font,       Font_Init,          u08 *FileData) \
    EXPORT(u32,        Font_GetGlyphIndex, font Font, u32 Codepoint) \
    EXPORT(font_glyph, Font_GetGlyph,      font Font, u32 Codepoint, r32 Scale)
...
internal vptr
Font_FindTable(font *Font, u32 Tag)
{
...
internal u32
Font_GetGlyphIndex(font Font, u32 Codepoint)
{
	u32 GlyphIndex = 0;
...
internal void
Font_FindSegments(msdf_shape *Shape, u08 *Data)
{
	...
	for (u32 F = 0, V = 0, C = 0; V < VertexCount; F++, V++) {
		...
		if (Flag & TTF_VERTEX_ON_CURVE) WasControl = FALSE;
		else if (!WasControl) {
			Shape->SegmentCount--;
			SegmentCounts[C]--;
			WasControl = TRUE;
	...
	if (NextOffset == Offset) {
		...
		for (u32 C = 0; C < Glyph.Shape.ContourCount; C++) {
			...
			for (u32 E = 0; E < Contour.EdgeCount; E++) {
				...
				Assert(
					(Contour.EdgeCount == 1 && Edge.Color == 0b111)
					|| Edge.Color == 0b101
					|| Edge.Color == 0b110
					|| Edge.Color == 0b011
				);
